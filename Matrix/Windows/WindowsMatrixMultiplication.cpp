...
static DWORD WINAPI multiply_block_windows(LPVOID lpParam) {
...
static long long threaded_block_multiply_win(const Matrix& a, const Matrix& b, Matrix& result, size_t block_size) {
    size_t n = a.size();
    ...
    for (size_t i = 0; i < n; i += block_size) {
        for (size_t j = 0; j < n; j += block_size) {
            for (size_t k = 0; k < n; k += block_size) {
                ...
                HANDLE hThread = CreateThread(NULL, 0, multiply_block_windows, data, 0, NULL);
    ...
    for (size_t i = 0; i < threads.size(); i += MAXIMUM_WAIT_OBJECTS) {
        DWORD count = (std::min)(static_cast<DWORD>(threads.size() - i), static_cast<DWORD>(MAXIMUM_WAIT_OBJECTS));
...
int main() {
    ...
    for (size_t block = 1; block <= n; ++block) {
        Matrix c_res(n);
        long long time = threaded_block_multiply_win(a, b, c_res, block);
